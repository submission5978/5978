...
###############
    ...
    def compare_sh(self, shn, shk, n)        
        ...
        shk2 = np.zeros(shk.shape)
        multimap3 = np.ones(shk.shape)
        for x in range (n-2):
            mean_shn = np.mean(shn[x])
            for i in range shk[0]:
                for j in range shk[1]:
                    if shn[x][i][j] >= shk[i][j]:
                        map1[i][j] = shn[x][i][j]
                    else:
                        map1[i][j] = 0
            ...
            multimap3 = np.minimum(map3,multimap3)
            
        for i in range shk[0]:
            for j in range shk[1]:
                if shk[i][j]>=multimap3[i][j]:   #
                    shk2[i][j]=shk[i][j]-multimap3[i][j]
                else:
                    shk2[i][j]=0
        shk2 = torch.from_numpy(shk2) # 
        return shk2
